...
export default [
  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      await sock.sendMessage(ctx.from, {
        text: [`🔮 *TRUTH*`, `${'─'.repeat(28)}`, ``, question, ``, `@${ctx.senderNumber} must answer! 👀`].join('\n'),
        mentions: [ctx.sender]
      }, { quoted: msg })
    }
  },

  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      await sock.sendMessage(ctx.from, {
        text: [`💥 *DARE*`, `${'─'.repeat(28)}`, ``, dare, ``, `@${ctx.senderNumber} must do this! 😂`].join('\n'),
        mentions: [ctx.sender]
      }, { quoted: msg })
    }
  },

  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      await sock.sendMessage(ctx.from, {
        text: [`🎱 *Magic 8-Ball*`, ``, `❓ _"${ctx.query}"_`, ``, response].join('\n')
      }, { quoted: msg })
    }
  },

  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      if (!choice || !RPS_OPTIONS.includes(choice)) {
        return sock.sendMessage(ctx.from, {
          text: [`🪨 *Rock Paper Scissors*`, ``, `Choose your weapon:`, `  ${ctx.prefix}rps rock`, `  ${ctx.prefix}rps paper`, `  ${ctx.prefix}rps scissors`].join('\n')
        }, { quoted: msg })
      }
      const botChoice = pick(RPS_OPTIONS)
      let result
      if (choice === botChoice) result = `🤝 *Draw!*`
      else if (RPS_BEATS[choice] === botChoice) result = `🏆 *You Win!*`
      else result = `💀 *You Lose!*`
      await sock.sendMessage(ctx.from, {
        text: [`🪨✂️📄 *Rock Paper Scissors*`, `${'─'.repeat(26)}`, ``, `You:  *${RPS_EMOJI[choice]} ${choice.toUpperCase()}*`, `Bot:  *${RPS_EMOJI[botChoice]} ${botChoice.toUpperCase()}*`, ``, result].join('\n')
      }, { quoted: msg })
    }
  },

  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      if (sides < 2 || sides > 100) {
        return sock.sendMessage(ctx.from, { text: `❌ Dice sides must be between 2 and 100.` }, { quoted: msg })
      }
      const result = Math.floor(Math.random() * sides) + 1
      await sock.sendMessage(ctx.from, {
        text: [
          `🎲 *Dice Roll* (${sides}-sided)`, ``, `Result: *${result}*`, ``,
          result === sides ? `🎉 Maximum roll!` : result === 1 ? `💀 Critical fail!` : result >= sides * 0.8 ? `🔥 Lucky roll!` : ``
        ].filter(Boolean).join('\n')
      }, { quoted: msg })
    }
  },

  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      await sock.sendMessage(ctx.from, { text: [`🪙 *Coin Flip*`, ``, `${emoji} *${result}!*`].join('\n') }, { quoted: msg })
    }
  },
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      if (sub === 'hint') {
        ...
        if (!session || session.type !== 'trivia') {
          return sock.sendMessage(ctx.from, { text: `❌ No active trivia. Start one with ${ctx.prefix}trivia` }, { quoted: msg })
        }
        return sock.sendMessage(ctx.from, { text: `💡 *Hint:* ${session.hint}` }, { quoted: msg })
      }

      if (sub === 'skip' || sub === 'answer') {
        const session = activeSessions.get(ctx.from)
        if (!session || session.type !== 'trivia') {
          return sock.sendMessage(ctx.from, { text: `❌ No active trivia.` }, { quoted: msg })
        }
        activeSessions.delete(ctx.from)
        return sock.sendMessage(ctx.from, { text: `✅ *Answer was:* ${session.answer.toUpperCase()}` }, { quoted: msg })
      }

      if (activeSessions.has(ctx.from) && activeSessions.get(ctx.from).type === 'trivia') {
        return sock.sendMessage(ctx.from, {
          text: [`⚠️ A trivia is already active!`, ``, `Just type your answer as a normal message.`, `Or: ${ctx.prefix}trivia hint — for a hint`, `Or: ${ctx.prefix}trivia skip — to skip`].join('\n')
        }, { quoted: msg })
      }

      const item = pick(TRIVIA)
      activeSessions.set(ctx.from, { type: 'trivia', answer: item.a, hint: item.hint, askedAt: Date.now(), askerJid: ctx.sender })

      setTimeout(() => {
        const s = activeSessions.get(ctx.from)
        if (s?.type === 'trivia' && s.askedAt === activeSessions.get(ctx.from)?.askedAt) {
          activeSessions.delete(ctx.from)
          sock.sendMessage(ctx.from, { text: `⏰ *Time's up!* Nobody answered.\n✅ Answer was: *${item.a.toUpperCase()}*` })
        }
      }, 60_000)

      await sock.sendMessage(ctx.from, {
        text: [`🧠 *TRIVIA*`, `${'─'.repeat(26)}`, ``, item.q, ``, `_Type your answer as a plain message!_`, `_${ctx.prefix}trivia hint — for a hint_`, `_${ctx.prefix}trivia skip — to skip_`, `_60 seconds to answer_`].join('\n')
      }, { quoted: msg })
    }
  },
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      if (sub === 'hint') {
        ...
        if (!session || session.type !== 'riddle') {
          return sock.sendMessage(ctx.from, { text: `❌ No active riddle. Start one with ${ctx.prefix}riddle` }, { quoted: msg })
        }
        return sock.sendMessage(ctx.from, { text: `💡 *Hint:* ${session.hint}` }, { quoted: msg })
      }
      ...
      if (sub === 'skip' || sub === 'answer') {
        const session = activeSessions.get(ctx.from)
        if (!session || session.type !== 'riddle') {
          return sock.sendMessage(ctx.from, { text: `❌ No active riddle.` }, { quoted: msg })
        }
        activeSessions.delete(ctx.from)
        return sock.sendMessage(ctx.from, { text: `✅ *Answer was:* ${session.answer.toUpperCase()}` }, { quoted: msg })
      }
      ...
      if (activeSessions.has(ctx.from) && activeSessions.get(ctx.from).type === 'riddle') {
        return sock.sendMessage(ctx.from, {
          text: [`⚠️ A riddle is already active!`, `Just type your answer as a message.`, `${ctx.prefix}riddle hint — for a hint`, `${ctx.prefix}riddle skip — to skip`].join('\n')
        }, { quoted: msg })
      }

      const item = pick(RIDDLES)
      activeSessions.set(ctx.from, { type: 'riddle', answer: item.a, hint: item.hint, askedAt: Date.now(), askerJid: ctx.sender })

      setTimeout(() => {
        const s = activeSessions.get(ctx.from)
        if (s?.type === 'riddle' && s.askedAt === activeSessions.get(ctx.from)?.askedAt) {
          activeSessions.delete(ctx.from)
          sock.sendMessage(ctx.from, { text: `⏰ *Time's up!* Nobody solved it.\n✅ Answer was: *${item.a.toUpperCase()}*` })
        }
      }, 90_000)

      await sock.sendMessage(ctx.from, {
        text: [`🧩 *RIDDLE*`, `${'─'.repeat(26)}`, ``, item.q, ``, `_Type your answer as a plain message!_`, `_${ctx.prefix}riddle hint — for a hint_`, `_${ctx.prefix}riddle skip — to skip_`, `_90 seconds to answer_`].join('\n')
      }, { quoted: msg })
    }
  },

  {
    ...
    handler: async (sock, msg, ctx, { api }) => {
      ...
      if (!ctx.query || ctx.query.toLowerCase() === 'start') {
        ...
        activeSessions.set(`guess_${ctx.from}_${ctx.sender}`, { type: 'guess', target, attempts: 0, askedAt: Date.now() })
        return sock.sendMessage(ctx.from, {
          text: [`🔢 *Number Guess Game*`, ``, `I'm thinking of a number between *1 and 100*.`, `You have 7 attempts!`, ``, `Type: ${ctx.prefix}guess <number>`].join('\n')
        }, { quoted: msg })
      }

      if (!session) {
        return sock.sendMessage(ctx.from, { text: `❌ Start a game first: ${ctx.prefix}guess start` }, { quoted: msg })
      }

      const guess = parseInt(ctx.query)
      if (isNaN(guess) || guess < 1 || guess > 100) {
        return sock.sendMessage(ctx.from, { text: `❌ Guess a number between 1 and 100.` }, { quoted: msg })
      }
      ...
      if (guess === session.target) {
        ...
        return sock.sendMessage(ctx.from, {
          text: [
            `🎉 *CORRECT!*`, ``, `The number was *${session.target}*!`, `You got it in *${session.attempts}* attempt(s)!`, ``,
            session.attempts === 1 ? `🤯 First try! Legendary!` : session.attempts <= 3 ? `🔥 Impressive!` : session.attempts <= 5 ? `👏 Nice job!` : `😅 You barely made it!`
          ].join('\n')
        }, { quoted: msg })
      }
      ...
      if (session.attempts >= 7) {
        activeSessions.delete(`guess_${ctx.from}_${ctx.sender}`)
        return sock.sendMessage(ctx.from, {
          text: [`💀 *GAME OVER!*`, ``, `The number was *${session.target}*.`, `Better luck next time! Try again: ${ctx.prefix}guess start`].join('\n')
        }, { quoted: msg })
      }

      const direction = guess < session.target ? '📈 Too low!' : '📉 Too high!'
      await sock.sendMessage(ctx.from, {
        text: [`${direction}`, ``, `Your guess: *${guess}*`, `Attempts left: *${remaining}*`].join('\n')
      }, { quoted: msg })
    }
  }
]
